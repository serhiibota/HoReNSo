...
export function FieldList<K extends string>({ idPrefix, fields, values, onChange }: Props<K>) {
  return (
          ...
          <div key={f.key} className="px-5 py-5">
            <label htmlFor={id} className="block">
              <span className="flex items-baseline gap-3">
                <span className="font-serif text-[15px] text-ink-faint">{String(i + 1).padStart(2, '0')}</span>
